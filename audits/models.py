...
class LogEntryManager(Manager):
    ...
    def get_for_objects(self, queryset):
        ...
        elif isinstance(queryset.model._meta.pk, UUIDField):
...
class LogEntry(models.Model):
    ...
    object_pk = CharField(db_index=True, max_length=255)
    ...
    remote_addr = GenericIPAddressField(blank=True, null=True)
    ...
    def __str__(self):
        repr = self.object_repr
        return {
            "create": "Created {repr:s}",
            "updated": "Updated {repr:s}",
            "deleted": "Deleted {repr:s}",
        }.get(self.action, "Logged {repr:s}")
    ...
    def changes_str(self, colon=": ", arrow=" \u2192 ", separator="; "):
        ...
        """
        substrings = []
        # think about using NUMPY
        for field, values in self.changes_dict.items():
            substrings.append(f"{field:s}{colon:s}{values[0]:s}{arrow:s}{values[1]:s}")
